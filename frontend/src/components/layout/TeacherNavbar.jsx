...
const TeacherNavbar = ({ onMenuClick, user, onApplyLeave }) => {
  ...
  // Click outside → close (no mark-read)
  useEffect(() => {
    const handler = (e) => {
  ...
  React.useEffect(() => {
    const fmt = () => {
      const now = new Date();
      setTime(now.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" }).toUpperCase() + " - " + now.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" }));
    };
    fmt();
    ...
    <>
      <header className="h-12 bg-white border-b border-gray-100 flex items-center px-4 gap-3 flex-shrink-0 z-10">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          <span className="hidden md:inline-flex items-center px-2 py-0.5 rounded-md bg-blue-50 text-blue-700 text-[10px] font-bold border border-blue-100">Mathematics</span>
            ...
            {notifOpen && (
              <div className="absolute right-0 top-9 w-80 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden">
                <div className="px-4 py-2.5 border-b border-gray-100 flex items-center justify-between">
                  ...
                  {unread > 0 && (
                    <button onClick={(e) => { e.stopPropagation(); markAllRead(); }} className="flex items-center gap-1 text-[10px] font-semibold text-blue-600 hover:underline">
                ...
                <div className="max-h-72 overflow-y-auto divide-y divide-gray-50">
                  {pendingReliefs.length > 0 && (
                    <div onClick={handleReliefClick} className="flex items-start gap-2.5 px-4 py-2.5 bg-orange-50 hover:bg-orange-100 cursor-pointer">
                      ...
                      <div>
                        <p className="text-[11px] font-bold text-orange-800">{pendingReliefs.length} Pending Relief Request{pendingReliefs.length > 1 ? "s" : ""}</p>
                    ...
                    notifs.slice(0, 10).map((n) => {
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-start gap-2.5 px-4 py-2.5 cursor-pointer hover:bg-gray-50 active:opacity-70 transition-colors ${!n.is_read ? 'bg-blue-50/40' : ''}`}
                        >
          ...
          <button className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"><Clock size={16} /></button>
          ...
          <div className="relative group ml-1">
            <div className="w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center text-white text-[10px] font-bold cursor-pointer select-none">{initials}</div>
      ...
      <ReliefAssignmentModal isOpen={reliefModalOpen} onClose={() => setReliefModalOpen(false)} assignment={activeAssignment} onAccept={handleAccept} onReject={handleReject} onFlag={handleFlag} />

      <AnimatePresence>
        {toastMessage && (
          <motion.div key="toast" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: 0.2 }}
