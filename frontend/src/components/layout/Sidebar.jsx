...
import {
  LayoutDashboard, CalendarDays, ClipboardCheck, UserCheck, BarChart3,
  HelpCircle, LogOut, GraduationCap, X, Home, Clock, FileText, Repeat2,
  LineChart, User, ShieldCheck, Users, Settings,
} from 'lucide-react';
...
const role = (user?.role ?? 'teacher').toLowerCase();
  ...
  return (
    <>
      ...
      {/* Sidebar shell
          Mobile:  fixed drawer, w-[80vw] max-w-[240px], slides in/out
          Desktop: static flex item, fixed w-52               */}
      <aside className={`
        fixed top-0 left-0 z-30 h-full w-[80vw] max-w-[240px] bg-white border-r border-gray-100
        flex flex-col transition-transform duration-200 ease-in-out
        ${open ? 'translate-x-0' : '-translate-x-full'}
        lg:!relative lg:w-52 lg:translate-x-0 lg:z-auto lg:shrink-0
      `}>
        {/* Brand */}
        <div className="flex items-center justify-between px-4 pt-5 pb-4 border-b border-gray-100">
          ...
          <button onClick={onClose} className="lg:hidden p-1 rounded-md text-gray-400 hover:text-gray-600" aria-label="Close sidebar">
            <X size={16} />
          </button>
        </div>
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
          {navItems.map(({ label, icon: Icon, to, end }) => (
            <NavLink key={to} to={to} end={end ?? false} onClick={onClose}
              className={({ isActive }) =>
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon size={15} className={isActive ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'} />
                  <span>{label}</span>
                </>
              )}
            </NavLink>
        ...
        {cta.label && (
          <div className="px-3 pb-3">
            <button onClick={handleCta}
              className="w-full py-2 px-3 bg-blue-600 text-white text-[11px] font-semibold rounded-lg hover:bg-blue-700 transition-colors">
              {cta.label}
            </button>
          </div>
        )}
