    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <div className="mb-4 p-3 bg-gray-50 rounded-xl text-sm text-gray-600">
          ...
          <p><span className="font-semibold">Day:</span> {DAY_FULL[slot.day_of_week]}, Period {slot.period}</p>
        </div>
        {error && (
...
export default function TimetableGrid() {
    ...
    try {
      ...
      if (deptId && Array.isArray(teachersData)) {
        ...
        const results = await Promise.allSettled(
          deptTeacherList.map(t => apiFetch(`/timetable/view?scope=teacher&scope_id=${t.id}`))
        );
        results.forEach(r => {
  ...
  const slotsByDay = DAY_FULL.map((_, dayIdx) =>
    slots.filter(s => Number(s.day_of_week) === dayIdx)
  );

  return (
    // Scrollable page wrapper for non-dashboard pages
    <div className="h-full overflow-y-auto px-3 sm:px-5 lg:px-6 py-4">
      <div className="space-y-5 max-w-6xl mx-auto">

        {/* Header */}
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-gray-900">Timetable</h1>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="border-collapse text-sm" style={{ width: '100%', minWidth: '480px' }}>
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50/80">
                    {/* Sticky period column */}
                    <th className="sticky left-0 z-10 bg-gray-50/80 w-16 sm:w-20 px-2 sm:px-4 py-3 text-center">
                      <Clock size={16} className="mx-auto text-gray-400" />
                    </th>
                    {DAY_FULL.map((d, i) => (
                      <th key={d} className="px-2 sm:px-3 py-3 text-center whitespace-nowrap">
                        {/* Short on mobile, full on sm+ */}
                        <p className="text-xs font-bold uppercase tracking-wider text-gray-700 sm:hidden">{DAY_SHORT[i]}</p>
                        <p className="text-xs font-bold uppercase tracking-wider text-gray-700 hidden sm:block">{d}</p>
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {periods.map((period) => (
                    <tr key={period} className="border-b border-gray-100">
                      {/* Sticky period label */}
                      <td className="sticky left-0 z-10 bg-white px-2 sm:px-4 py-2 text-center text-xs font-bold text-gray-500 whitespace-nowrap border-r border-gray-100">
                        <span className="hidden sm:inline">Period </span>{period}
                      </td>
                      {DAY_FULL.map((_, dayIdx) => {
                        const slot = slotsByDay[dayIdx]?.find(s => s.period === period);
                        return (
                          <td key={dayIdx} className="px-1 sm:px-2 py-2">
                            {slot ? (
                              <div onClick={() => handleSlotClick(slot)}
                                className={`relative flex min-h-[60px] sm:min-h-[72px] flex-col justify-center rounded-lg border border-gray-100 bg-white border-l-[3px] border-l-indigo-500 px-2 sm:px-3 py-2 transition ${isHOD ? 'cursor-pointer hover:shadow-md hover:border-indigo-300 group' : ''}`}>
                                <p className="text-[11px] sm:text-[12px] font-bold leading-tight text-indigo-700 line-clamp-2">
                                  {slot.subject || subjectMap[slot.subject_id] || "Subject"}
                                </p>
                                <p className="mt-0.5 text-[10px] sm:text-[11px] text-gray-500 truncate">
                                  {slot.room || teacherMap[slot.teacher_id] || "Teacher"}
                                </p>
                                <p className="mt-0.5 text-[9px] sm:text-[10px] text-gray-400 truncate hidden sm:block">
                              ...
                              <div className="flex h-full min-h-[60px] sm:min-h-[72px] items-center justify-center rounded-lg border border-dashed border-gray-300 bg-white">
