...
import { AlertCircle, Plus, ArrowRight, MoreVertical } from 'lucide-react';
...
const HODDashboard = () => {
  ...
  const statCards = [
    { value: loading ? '…' : (stats?.pending_approvals_count ?? 0), label: 'Pending Leave Approvals', badge: 'PENDING', badgeColor: 'bg-blue-100 text-blue-600',    borderColor: 'border-blue-200'    },
    { value: loading ? '…' : (stats?.missing_reliefs ?? 0),         label: 'Missing Reliefs',         badge: 'URGENT',  badgeColor: 'bg-red-100 text-red-600',      borderColor: 'border-red-200'     },
    { value: loading ? '…' : (stats?.total_teachers ?? 0),          label: 'Dept Teachers',           badge: 'ACTIVE',  badgeColor: 'bg-emerald-100 text-emerald-600', borderColor: 'border-emerald-200' },
    { value: loading ? '…' : (stats?.active_conflicts ?? 0),        label: 'Active Conflicts',        badge: 'CLEAR',   badgeColor: 'bg-green-100 text-green-600',  borderColor: 'border-green-200'   },
  ];
  ...
  return (
    ...
    <div className="relative flex flex-col h-full min-h-0">
      ...
      {/* Scrollable zone */}
      <div className="flex-1 overflow-y-auto pb-[96px] px-3 sm:px-5 lg:px-6 py-4 space-y-5 max-w-6xl w-full mx-auto">
        ...
        {/* Quick Actions — desktop only inline card */}
        <div className="hidden lg:block bg-white border border-gray-100 rounded-xl shadow-sm p-4">
          <h3 className="text-[13px] font-semibold text-gray-800 mb-3">Quick Actions</h3>
          <div className="grid grid-cols-4 gap-3">
            {HOD_QUICK_ACTIONS.map((qa) => {
              const Icon = qa.icon;
              return (
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-4">
              ...
              {stats?.missing_reliefs > 0 ? (
                <div className="flex items-center justify-between bg-red-50 border border-red-100 rounded-lg px-3 py-2.5 gap-3 flex-wrap">
                  <div>
                  ...
                  <button onClick={() => navigate('/hod/relief')}
                    className="px-3 py-1.5 bg-blue-600 text-white text-[11px] font-semibold rounded-lg whitespace-nowrap">
                    Assign Now
                  </button>
                </div>
              ) : (
                ...
                <table className="w-full min-w-[360px]">
                  <thead>
                    <tr className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">
                      ...
                      <th className="text-left pb-2 pr-4 hidden sm:table-cell">Subject</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {MOCK_TIMETABLE.map((row, i) => (
                      <tr key={i} className={`text-[12px] ${row.status === 'cancelled' ? 'opacity-50' : ''}`}>
                        <td className="py-2 pr-4 font-semibold text-gray-600 whitespace-nowrap">{row.period}<span className="text-gray-400 font-normal ml-1 hidden sm:inline">({row.time})</span></td>
                        <td className="py-2 pr-4 text-gray-700 whitespace-nowrap">{row.teacher}</td>
                        <td className="py-2 pr-4 text-gray-700 hidden sm:table-cell">{row.subject}</td>
