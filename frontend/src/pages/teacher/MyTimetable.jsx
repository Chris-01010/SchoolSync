...
const CELL_CONFIG = {
  ...
  free:      { bg: 'bg-gray-50 border-gray-200',              text: 'text-gray-400',  sub: 'text-gray-300',  badge: null, freeIcon: true },
...
export default function MyTimetable() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Step 2: find matching teacher profile
      ...
      // Step 3: fetch timetable with correct scope params
      const ttResp = await api.get(`/timetable/view?scope=teacher&scope_id=${myTeacher.id}`);

      // Response shape: { scope, scope_id, timetable: { "0": [...], "1": [...] } }
      const rawTimetable = ttResp?.timetable ?? ttResp ?? {};

      // Normalise: if it came back as an array, group by day_of_week
  ...
  const buildCell = useCallback((dayIndex, period) => {
    ...
    return {
      type,
      // subject field: try subject string directly, then subject_name, then subject_id fallback
  ...
  return (
    <div className="space-y-4 max-w-[1280px]">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="flex items-center gap-1.5 px-3 py-2 border border-gray-200 rounded-lg text-[11px] font-semibold text-gray-600 hover:bg-gray-50 transition-colors bg-white">
                    ...
                    return (
                      <div key={`break-${si}`} className="grid border-b border-gray-100 bg-gray-50"
                        ...
                        <div className="flex items-center justify-center py-2 gap-2">
                          ...
                          <span className="text-[9px] text-gray-400 font-medium">· {slot.start} – {slot.end}</span>
                        </div>
                      </div>
                    );
                  }
