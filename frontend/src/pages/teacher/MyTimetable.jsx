...
import { useBlockSync } from '../../context/BlockSyncContext';  // ← ADDED
...
export default function MyTimetable() {
  ...
  const [blockedTooltip, setBlockedTooltip] = useState(null);
  const cellRefs                            = useRef({});
  const [toast, setToast]                   = useState(null);

  // ── ADDED: pull notifyBlockChange from shared context ─────────────────────
  const { notifyBlockChange } = useBlockSync();
  ...
  const handleConfirmBlock = useCallback(async (reason, clarification) => {
    ...
    try {
      ...
      notifyBlockChange();  // ← ADDED: HOD grid will refetch
    } catch (err) {
      setToast({ message: err.message || 'Failed to block slot.', type: 'error' });
    } finally {
      setBlockLoading(false);
    }
  }, [blockPopup, teacherId, notifyBlockChange]);
  ...
  }, [blockedTooltip, notifyBlockChange]);
