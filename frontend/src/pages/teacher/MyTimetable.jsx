...
function computeWeekDays(today) {
  ...
  return Array.from({ length: 5 }, (_, i) => {
    ...
    return { label: `${DAY_NAMES[i]} (${d.getDate()})`, dayIndex: i, date: d };
  });
}
...
const TimetableCell = ({ cell }) => {
  if (!cell) {
    return (
      <div className="h-[68px] bg-gray-50 border border-gray-100 rounded-lg flex items-center justify-center">
  ...
  if (cell.type === 'free') {
    return (
      <div className={`h-[68px] border rounded-lg flex flex-col items-center justify-center gap-1 ${cfg.bg}`}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-300">
  ...
  return (
    <div className={`h-[68px] border rounded-lg p-1.5 flex flex-col justify-between overflow-hidden ${cfg.bg}`}>
      ...
      <div>
        ...
        {cell.originalTeacher && (
          <p className={`text-[9px] truncate ${cfg.sub}`}>for {cell.originalTeacher}</p>
        )}
      </div>
    </div>
  );
...
export default function MyTimetable() {
  ...
  }, [slotMap, todayBackendDow, nowMinutes]);
  ...
  // ── Column widths ──────────────────────────────────────────────────────────
  // Time column: 56px fixed, sticky.
  // Each day column: at least 100px so 5 cols = 556px min — overflows on small screens.
  const TIME_COL  = 56;
  const DAY_COL   = 100;  // px, minimum per day column
  const totalMinW = TIME_COL + DAY_COL * visibleDays.length;
  // gridTemplateColumns for the inner table
  const gridCols  = `${TIME_COL}px repeat(${visibleDays.length}, minmax(${DAY_COL}px, 1fr))`;

  return (
    // Page scroll wrapper — fills layout container
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_220px] gap-4">
          ...
          >
            {loading ? (
              ...
              // overflow-x-auto lets the grid scroll horizontally on small screens.
              // The inner div has a min-width so columns never collapse below DAY_COL.
                    ...
                    return (
                      <div key={slot.period}
                        ...
                        {visibleDays.map((d) => {
                          ...
                          const isToday = d.dayIndex === todayBackendDow;
                          const match   = !search || (cell.type !== 'free' &&
