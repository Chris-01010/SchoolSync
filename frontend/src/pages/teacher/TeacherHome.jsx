  ...
  <div className="bg-white border border-gray-100 rounded-xl p-3.5 shadow-sm min-w-[200px]">
    <div className="flex items-center gap-2 mb-2">
      ...
      <div className="min-w-0">
        ...
        {duty.department && <p className="text-[9px] text-gray-400 truncate">{duty.department}</p>}
      </div>
      <span className="ml-auto text-[9px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-md whitespace-nowrap">Period {duty.period}</span>
    </div>
    ...
    <div className="flex gap-2 mt-2.5">
      {duty.lessonPlanUrl && (
        <button className="flex-1 py-1.5 bg-blue-600 text-white text-[10px] font-semibold rounded-lg hover:bg-blue-700 transition-colors">View Lesson Plan</button>
      )}
      <button className="p-1.5 border border-gray-200 rounded-lg text-gray-400 hover:bg-gray-50 transition-colors"><MoreHorizontal size={12} /></button>
    </div>
    {duty.upcoming && <p className="text-[9px] text-gray-400 text-center mt-1.5">Upcoming</p>}
  </div>
);
  ...
  return (
    <div className="space-y-5 max-w-[1280px]">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
        ...
        <div>
          <h1 className="text-[20px] font-bold text-blue-600 leading-tight">Welcome back, {teacher.name}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="p-3 space-y-2">
            {todaySchedule.map((item) => <ScheduleRow key={item.id} item={item} />)}
          </div>
        </div>
