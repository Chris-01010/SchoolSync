...
const DonutChart = ({ current, total }) => {
  ...
  const data = [{ name: 'Done', value: current }, { name: 'Left', value: Math.max(0, safeTotal - current) }];
  return (
    <div className="relative w-[140px] h-[140px] mx-auto">
      <PieChart width={140} height={140}>
        <Pie data={data} cx={70} cy={70} innerRadius={48} outerRadius={64}
          startAngle={90} endAngle={-270} dataKey="value" strokeWidth={0}>
          <Cell fill="#3b82f6" /><Cell fill="#e0e7ff" />
        </Pie>
      </PieChart>
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <p className="text-[20px] font-bold text-gray-900 leading-none">{current}/{total}</p>
...
const WorkloadBar = ({ teaching, relief, free }) => {
  ...
  const segments = [
    { pct: (teaching / total) * 100, color: 'bg-blue-600',   tooltip: 'Teaching', label: `${teaching}h` },
    { pct: (relief  / total) * 100, color: 'bg-orange-400', tooltip: 'Relief',   label: `${relief}h`   },
    { pct: (free    / total) * 100, color: 'bg-gray-200',   tooltip: 'Free',     label: `${free}h`     },
  ];
  return (
    <div className="space-y-2">
      <div className="flex h-6 rounded-full overflow-hidden gap-0.5">
        {segments.map((s) => (
          <div key={s.tooltip} className={`${s.color} flex items-center justify-center transition-all`}
            style={{ width: `${s.pct}%` }} title={`${s.tooltip}: ${s.label}`}>
            {s.pct > 12 && <span className="text-[9px] font-bold text-white">{s.label}</span>}
          </div>
        ))}
      </div>
      <div className="flex items-center gap-4">
        {[{ color: 'bg-blue-600', label: 'Teaching' }, { color: 'bg-orange-400', label: 'Relief' }, { color: 'bg-gray-300', label: 'Free' }].map((l) => (
...
export default function MyWorkload() {
  ...
  const s = useMemo(() => deriveWorkload(timetableData ?? {}, profile), [timetableData, profile]);
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex bg-gray-100 rounded-lg p-0.5 gap-0.5">
            {TIME_FILTERS.map((f, i) => (
              <button key={f} onClick={() => setTimeFilter(i)} disabled={i !== 0}
                className={`px-3 py-1.5 text-[11px] font-semibold rounded-md transition-all ${
                  timeFilter === i ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-400 cursor-not-allowed'
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-[10px] font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-md">{s.deptName}</span>
                </div>
                <WorkloadBar teaching={s.weeklyBreakdown.teaching} relief={s.weeklyBreakdown.relief} free={s.weeklyBreakdown.free} />
                <div className="mt-5 bg-blue-600 rounded-xl p-4 text-white">
                  <p className="text-[10px] font-bold text-blue-200 uppercase tracking-wider mb-1">Total Scheduled</p>
                  <p className="text-[28px] font-bold leading-none">{s.weeklyBreakdown.teaching + s.weeklyBreakdown.relief}h</p>
                  <p className="text-[10px] text-blue-200 mt-1">{s.weeklyBreakdown.teaching}h teaching + {s.weeklyBreakdown.relief}h relief</p>
                  <button className="mt-3 text-[10px] font-bold text-white underline underline-offset-2 hover:text-blue-200 transition-colors flex items-center gap-1">
              ...
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={s.dailyHours} barSize={28}>
                  ...
                  <Bar dataKey="hours" radius={[6, 6, 0, 0]}>
                    {s.dailyHours.map((_, index) => (
                      <Cell key={`cell-${index}`} fill={index === busiestDayIndex ? '#3b82f6' : '#bfdbfe'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
