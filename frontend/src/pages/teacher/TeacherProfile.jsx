...
export function TeacherProfile() {
  ...
  // ─── Sync Backend Data → Editable Form ───────────────────────────────────
  ...
  const handleEditSave =
    async () => {
      try {
        ...
        // backend update hook
        // TODO:
        // Replace endpoint once PATCH profile API exists

        try {
          if (profile?.id) {
            await api.put(
              `/api/v1/teachers/${profile.id}`,
              {
                name:
                  editValues.name,
              }
          ...
          console.warn(
            'Teacher profile update endpoint unavailable:',
            backendErr
          );
        }
      } catch (err) {
  ...
  const prefItems = [
...

    {
      key: 'emailRelief',
      label:
        'Email notifications for relief assignments',
    },

    {
      key: 'dailyEmail',
      label:
        'Daily schedule email (8:00 AM)',
    },
  ];
      ...
      <div className="flex items-center justify-between">
        ...
        {!editing ? (
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-2 border border-gray-200 rounded-lg text-[11px] font-semibold text-gray-600 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Pencil size={12} />
            Edit
          </button>
        ...
        <>
          ...
          >
            ...
            <p className="text-[10px] text-gray-400 mt-4 italic">
              Preferences are currently local-only and will reset on refresh.
            </p>
          </motion.div>
          ...
          <button
            onClick={() =>
              alert(
                'Change Password feature will be connected to backend soon.'
              )
            }
