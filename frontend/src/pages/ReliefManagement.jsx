...
const statusConfig = {
  ...
  unassigned: {
    label: "Unassigned",
    pill: "bg-amber-50 text-amber-700 ring-amber-500/20",
    dot: "bg-amber-500",
  },
  overridden: {
    label: "Overridden",
    pill: "bg-purple-50 text-purple-700 ring-purple-500/20",
    dot: "bg-purple-500",
  },
};
...
export default function ReliefManagementPage() {
  ...
  const fetchReliefs = async () => {
    ...
    try {
      ...
      // fallback mock data if backend empty
      if (!list.length) {
        list = [
          {
            id: "p2",
          ...
          },
          {
            ...
            period: "Period 1",
            class_name: "10A",
            absent_teacher: "Prof. Kumar",
            substitute_name: "Mr. Smith",
            substitute_id: "teacher_001",
            status: "assigned",
          },
        ];
      }
      ...
      setError(
        err?.message || "Failed to load relief data."
      );
    } finally {
      setLoading(false);
    }
  ...
  const assignedCount = reliefs.filter(
    (r) => r.status === "assigned"
  ).length;

  const unassignedCount = reliefs.filter(
    (r) =>
      r.status === "unassigned" ||
      !r.substitute_id
  ).length;

  const overrideCount = reliefs.filter(
    (r) =>
      r.override ||
      r.status === "overridden"
  ).length;
  ...
  const summaryStats = [
    {
      label: "TOTAL RELIEF TODAY",
      value: loading ? "—" : totalRelief,
    ...
    },
    {
      ...
      value: loading ? "—" : overrideCount,
      sub: "Overridden by admin",
  ...
  const handleAssignRelief = async () => {
    ...
    const teacher =
      ...
      fallbackTeachers[
        Math.floor(
          Math.random() * fallbackTeachers.length
        )
      ];
    ...
    try {
      ...
      const updated = reliefs.map((r) => {
        if (r.id !== modalRow.id) return r;

        return {
          ...r,
          substitute_name: teacher,
          substitute_id: teacher,
          override: overrideAllocation,
          notes,
          status: overrideAllocation
            ? "overridden"
            : "assigned",
        };
      });
      ...
      // backend hook if endpoint exists
      try {
        await api.put(
          `/leaves/relief/${modalRow.id}/override`,
          {
            relief_teacher_id:
              modalRow.relief_teacher_id ||
              modalRow.substitute_id ||
              modalRow.teacher_id,
            override_note: notes,
          }
        );
        ...
        console.warn(
          "Backend override endpoint unavailable:",
          backendErr
        );
      }
        ...
        >
          {summaryStats.map((s) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg ${s.iconBg} ${s.accent}`}
                >
                  <Icon size={18} />
                </div>
                ...
                <div>
                  ...
                  <p
                    className={`mt-0.5 text-[28px] font-bold leading-none tracking-tight ${s.accent}`}
                  >
                    {s.value}
                  </p>
          ...
          {error && (
            <div className="flex items-center justify-between border-b border-red-100 bg-red-50 px-5 py-4">
              ...
              <button
                ...
                className="ml-4 rounded-lg bg-red-600 px-3.5 py-1.5 text-xs font-semibold text-white transition hover:bg-red-700"
              >
                Retry
              </button>
            </div>
                ...
                <tbody>
                  {reliefs.map((r) => {
                    ...
                    return (
                      ...
                      >
                        <td className="whitespace-nowrap px-5 py-3.5 text-[13px] font-medium text-gray-700">
                          {r.department || "—"}
                        </td>
                        ...
                        <td className="whitespace-nowrap px-5 py-3.5">
                          <span
                            className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-semibold ring-1 ring-inset ${sc.pill}`}
                          >
                            <span
            ...
            <div className="flex items-center justify-between border-t border-gray-200 px-5 py-3">
              <p className="text-xs text-gray-500">
                ...
                <span className="font-semibold text-purple-600">
                  {overrideCount} overridden by Admin
                </span>
              </p>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full appearance-none rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm"
                    >
                      <option value="">
                        -- Auto-assign --
                      </option>

                      {fallbackTeachers.map((t) => (
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full resize-none rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 placeholder-gray-400 shadow-sm"
                  />
                </div>
              </div>
