...
const alerts = [
  ...
  { color: "bg-red-500",   title: "4 Unassigned Reliefs",        description: "Critical shortage for Period 4. No relief teachers have accepted.", time: "15 mins ago" },
...
export default function AdminHome() {
  ...
  return (
    // Outer shell — fills layout, owns scroll + fixed bar
    <div className="relative flex flex-col h-full min-h-0">

      {/* Scrollable zone */}
      <div className="flex-1 overflow-y-auto pb-[96px] lg:pb-6 px-3 sm:px-5 lg:px-6 py-4 space-y-6">

        {/* Header */}
        <motion.section variants={itemVariants} className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-gray-900">Welcome back, Admin</h1>
            <p className="mt-1 text-sm text-gray-500">Manage institutional timetables and teacher reliefs with precision.</p>
          </div>
          <div className="mt-2 flex items-center gap-2 sm:mt-0 flex-wrap">
        ...
        <motion.section variants={itemVariants} initial="hidden" animate="visible">
          ...
          {error && (
            <div className="flex items-center justify-between rounded-xl border border-red-200 bg-red-50 px-5 py-4 flex-wrap gap-3">
              <p className="text-sm font-medium text-red-600">{error}</p>
              <button onClick={fetchStats} className="rounded-lg bg-red-600 px-3.5 py-1.5 text-xs font-semibold text-white hover:bg-red-700 transition">Retry</button>
            </div>
          )}
          {!loading && !error && (
        ...
        {/* Alerts + Quick Actions (desktop inline) */}
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <motion.section variants={itemVariants} className="flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3.5 flex-wrap gap-2">
          ...
          {/* Quick Actions — desktop only inline */}
          <motion.section variants={itemVariants} className="hidden lg:flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <motion.section variants={itemVariants} className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3.5">
            ...
            <button className="text-[13px] font-semibold text-indigo-600 transition hover:text-indigo-800 whitespace-nowrap">View All</button>
          </div>
        ...
        {/* Footer */}
        <footer className="border-t border-gray-200 pt-5 pb-4 text-center text-xs text-gray-400">
      ...
      {/* Fixed Quick Actions bar — mobile & tablet only */}
      <div className="lg:hidden absolute bottom-0 left-0 right-0 z-10 bg-white border-t border-gray-100 shadow-[0_-2px_12px_rgba(0,0,0,0.06)] px-4 py-2.5">
          ...
          {ADMIN_QUICK_ACTIONS.map((qa) => {
            ...
            return (
              <motion.button key={qa.label} onClick={() => navigate(qa.path)}
                whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                transition={{ type: "spring", stiffness: 400, damping: 22 }}
                className="group flex flex-col items-center justify-center gap-1.5 rounded-xl border border-gray-200 bg-white px-2 py-3 text-center transition-colors hover:border-indigo-300 hover:bg-indigo-50/50">
                <Icon size={20} strokeWidth={1.8} className="text-indigo-500 transition group-hover:text-indigo-700" />
                <span className="text-[10px] font-semibold text-gray-700 group-hover:text-indigo-700 leading-tight">{qa.label}</span>
              </motion.button>
            );
          })}
        </div>
