...
import {
  Save, Building, Calendar, ClipboardList, Bell, AlertTriangle,
  Camera, Download, RotateCcw, Server, Clock, Shield,
} from "lucide-react";
...
function ToggleSwitch({ checked, onChange, id }) {
  return (
    <button type="button" id={id} role="switch" aria-checked={checked}
      onClick={() => onChange(!checked)}
      className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:ring-offset-2 ${checked ? "bg-indigo-600" : "bg-gray-200"}`}>
...
function FieldLabel({ children }) {
  return <label className="mb-1.5 block text-[11px] font-semibold uppercase tracking-wider text-gray-400">{children}</label>;
}

const inputCls = "w-full rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-sm text-gray-700 placeholder-gray-400 shadow-sm transition focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20";

function SectionCard({ icon: Icon, iconBg, iconColor, title, children, className = "" }) {
  return (
...
export default function SettingsPage() {
  ...
  /* Leave & Relief — wired to real backend */
  const [maxWeeklyHours, setMaxWeeklyHours]   = useState(30);
  const [maxReliefPerWeek, setMaxReliefPerWeek] = useState(3);
  const [fairnessFactor, setFairnessFactor]   = useState(0.5);
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (data) {
        setMaxWeeklyHours(data.max_weekly_hours_default ?? 30);
        setMaxReliefPerWeek(data.weekly_relief_cap_default ?? 3);
        setFairnessFactor(data.fairness_balance_factor ?? 0.5);
        setHodApproval(data.allow_hod_auto_approval ?? true);
      }
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put("/admin/settings", {
        max_weekly_hours_default: maxWeeklyHours,
        weekly_relief_cap_default: maxReliefPerWeek,
        fairness_balance_factor: fairnessFactor,
        allow_hod_auto_approval: hodApproval,
      });
      setSaveSuccess(true);
    ...
    <motion.div variants={containerV} initial="hidden" animate="visible" className="space-y-7">
      ...
      <motion.div variants={itemV} className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[14px] text-gray-500">Configure system preferences and institutional settings.</p>
        </div>
        <button onClick={handleSave} disabled={saving}
          className="inline-flex flex-shrink-0 items-center gap-1.5 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-indigo-700 disabled:opacity-60">
          <Save size={15} />
      ...
      {error && (
        <motion.div variants={itemV} className="flex items-center justify-between rounded-xl border border-red-200 bg-red-50 px-5 py-4">
          ...
          <button onClick={fetchSettings} className="ml-4 rounded-lg bg-red-600 px-3.5 py-1.5 text-xs font-semibold text-white hover:bg-red-700 transition">Retry</button>
        </motion.div>
      )}

      {/* SAVE ERROR */}
      {saveError && (
        <motion.div variants={itemV} className="rounded-xl border border-red-200 bg-red-50 px-5 py-3">
      ...
      {/* SAVE SUCCESS */}
      {saveSuccess && (
        <motion.div variants={itemV} className="rounded-xl border border-emerald-200 bg-emerald-50 px-5 py-3">
      ...
      <motion.div variants={itemV} className="grid grid-cols-1 gap-6 xl:grid-cols-[65%_1fr]">
        ...
        <div className="space-y-6">
          ...
          <SectionCard icon={Building} iconBg="bg-indigo-50" iconColor="text-indigo-600" title="Institution Profile">
            <div className="space-y-4">
              ...
              <div>
                ...
                <select value={academicYear} onChange={(e) => setAcademicYear(e.target.value)} className={`${inputCls} appearance-none`}>
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input type="number" value={periodDuration} onChange={(e) => setPeriodDuration(Number(e.target.value))} className={inputCls} />
                    <span className="flex-shrink-0 text-[12px] font-medium text-gray-400">min</span>
                  </div>
                </div>
                <div>
                  <FieldLabel>Periods per Day</FieldLabel>
                  <input type="number" value={periodsPerDay} onChange={(e) => setPeriodsPerDay(Number(e.target.value))} className={inputCls} />
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(workingDays).map(([day, active]) => (
                    <button key={day} type="button" onClick={() => toggleDay(day)}
                      className={`rounded-lg border px-4 py-2 text-[13px] font-semibold transition-all ${active ? "border-indigo-400 bg-indigo-50 text-indigo-700 ring-1 ring-indigo-400/30" : "border-gray-200 bg-white text-gray-500 hover:border-gray-300 hover:bg-gray-50"}`}>
                      {day}
                    </button>
                  ))}
                </div>
                ...
                {breakEnabled && (
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input type="number" value={breakDuration} onChange={(e) => setBreakDuration(Number(e.target.value))} className={inputCls} />
                    </div>
                  </div>
                )}
              </div>
          ...
          {/* Leave & Relief Settings — WIRED */}
          <SectionCard icon={ClipboardList} iconBg="bg-emerald-50" iconColor="text-emerald-600" title="Leave & Relief Settings">
            <div className="space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <FieldLabel>Max Weekly Hours per Teacher</FieldLabel>
                  <input type="number" value={maxWeeklyHours}
                    onChange={(e) => setMaxWeeklyHours(Number(e.target.value))} className={inputCls} />
                </div>
                <div>
                  <FieldLabel>Require Medical Certificate after</FieldLabel>
                  <div className="flex items-center gap-2">
                    <input type="number" value={medCertAfter}
                      onChange={(e) => setMedCertAfter(Number(e.target.value))} className={inputCls} />
              ...
              <div className="space-y-3 rounded-lg border border-gray-200 bg-gray-50/50 p-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[12px] text-gray-400">Balance relief duties equally (factor: {fairnessFactor})</p>
                  </div>
                  <ToggleSwitch checked={fairnessFactor > 0} onChange={(v) => setFairnessFactor(v ? 0.5 : 0)} id="toggle-fairness" />
                </div>
                <div className="border-t border-gray-200" />
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-700">HOD Auto-Approval</p>
                    <p className="text-[12px] text-gray-400">Allow HOD to auto-approve leave requests</p>
                  </div>
              ...
              <div>
                ...
                <input type="number" value={maxReliefPerWeek}
            ...
            <div className="space-y-0 divide-y divide-gray-100">
              {notifItems.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between py-3.5 first:pt-0 last:pb-0">
                  ...
                  <ToggleSwitch checked={item.checked} onChange={item.onChange} id={`toggle-notif-${idx}`} />
                </div>
              ))}
            </div>
          </SectionCard>
        ...
        <div className="space-y-6">
          ...
          <SectionCard icon={Shield} iconBg="bg-indigo-50" iconColor="text-indigo-600" title="Admin Profile">
            <div className="space-y-4">
              ...
              <button className="flex w-full items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700">
            ...
            <div className="divide-y divide-gray-100">
              ...
              <div className="flex items-center justify-between py-3">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-700 ring-1 ring-inset ring-emerald-500/20">
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-emerald-500" /> Operational
                </span>
              </div>
              <div className="py-3">
                <div className="flex items-center justify-between">
            ...
            <div className="divide-y divide-red-200/60">
              <div className="flex items-center justify-between gap-4 pb-4">
                <div>
                  ...
                  <p className="mt-0.5 text-[12px] text-gray-500">Permanently clear all timetable data for the current academic year.</p>
                </div>
                <button className="flex-shrink-0 rounded-lg border border-red-300 bg-white px-4 py-2 text-[13px] font-semibold text-red-600 transition hover:bg-red-50 hover:border-red-400">
                  <span className="flex items-center gap-1.5"><RotateCcw size={13} /> Reset</span>
                </button>
              </div>
              <div className="flex items-center justify-between gap-4 pt-4">
                <div>
                  <p className="text-[13px] font-bold text-gray-800">Export All Data</p>
                  <p className="mt-0.5 text-[12px] text-gray-500">Download a complete backup of all institutional data.</p>
                </div>
                <button className="flex-shrink-0 rounded-lg border border-gray-300 bg-white px-4 py-2 text-[13px] font-semibold text-gray-600 transition hover:bg-gray-50 hover:border-gray-400">
                  <span className="flex items-center gap-1.5"><Download size={13} /> Export</span>
                </button>
              </div>
            </div>
          </div>
