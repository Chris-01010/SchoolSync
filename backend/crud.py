...
# ─── MUST be before /teachers/{teacher_id} so FastAPI doesn't parse "me" as UUID ───

@router.get("/teachers/me", response_model=TeacherSchema, tags=["Teachers"])
...
@router.get("/subjects", response_model=List[SubjectSchema], tags=["Subjects"])
async def list_subjects(db: AsyncSession = Depends(get_db), current_user: User = Depends(auth.get_current_user)):
    result = await db.execute(select(Subject))
    return result.scalars().all()


@router.get("/subjects/{subject_id}", response_model=SubjectSchema, tags=["Subjects"])
