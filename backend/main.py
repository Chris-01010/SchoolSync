...
async def get_timetable_view(
    ...
    query = select(models.TimetableSlot).join(models.TimetableVersion).where(
    ...
    # Fetch lookup maps for subject, room, class names
    subjects_res = await db.execute(select(models.Subject))
    subject_map  = {str(s.id): s.name for s in subjects_res.scalars().all()}

    rooms_res = await db.execute(select(models.Room))
    room_map  = {str(r.id): r.name for r in rooms_res.scalars().all()}

    classes_res = await db.execute(select(models.ClassRoom))
    class_map   = {str(c.id): c.name for c in classes_res.scalars().all()}
    ...
    for slot in slots:
        ...
        grouped[day].append({
            ...
            "subject":      subject_map.get(str(slot.subject_id), "Unknown"),
            "room":         room_map.get(str(slot.room_id), ""),
            "class_name":   class_map.get(str(slot.class_id), ""),
...
async def generate_timetable_sync(db: AsyncSession = Depends(get_db)):
    ...
    if not teachers: raise HTTPException(status_code=400, detail="No teachers found in database")
    if not rooms:    raise HTTPException(status_code=400, detail="No rooms found in database")
    if not subjects: raise HTTPException(status_code=400, detail="No subjects found in database")
    if not classes:  raise HTTPException(status_code=400, detail="No classes found in database")
    ...
    # Nullify slot references in relief_assignments first, then delete slots
    await db.execute(
        models.ReliefAssignment.__table__.update().values(slot_id=None)
    )
    ...
    await db.commit()
    return {"status": "success", "task_id": str(version.id), "version_id": str(version.id), "slots_created": slots_created}
